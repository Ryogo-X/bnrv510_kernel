...
#include "ntx-misc.h"
	...
	#include "../../arch/arm/mach-mx6/board-mx6q_ntx.h"
#elif defined (CONFIG_MACH_MX6SL_NTX)//][!CONFIG_MACH_MX6Q_NTX
...
static int g_is_power_off;
static int g_is_reboot;
...
static void pmic_power_off (void)
{
	printk ("%s()\n",__func__);
	g_is_power_off = 1;
	queue_delayed_work(chg_wq, &chg_work, 1000);
}

static void pmic_restart (void)
{
	printk ("%s()\n",__func__);
	g_is_reboot = 1;
...
#ifdef CONFIG_MACH_MX6Q_NTX//[
static ssize_t cash_drawer_info(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	*buf = (char)(gpio_get_value (ntx_misc->cd_open_gpio));
	printk ("%s() : Cash drawer status -- %s\n",__func__,(*buf?"unlocked":"locked"));
	return 1;
}

static ssize_t cash_drawer_ctl(struct device *dev, struct device_attribute *attr,
		       const char *buf, size_t count)
{
	int volt, period;
	char *pPeriod, *pVolt = buf;
    if (pPeriod = strchr (buf, ' ')) {
    	*pPeriod = '\0';
    	pPeriod++;
 		volt = simple_strtoul(pVolt, NULL, 10);
 		period = simple_strtoul(pPeriod, NULL, 10);
    	printk ("Setting cash drawer %dv, %dms\n",volt, period);   	
		if (12 == volt) {
			gpio_direction_output (ntx_misc->cd_N12V_EN_gpio, 1);
			gpio_direction_output (ntx_misc->cd_24V_gpio, 1);
			gpio_direction_output (ntx_misc->cd_12V_gpio, 0);
			msleep (period);
			gpio_direction_output (ntx_misc->cd_12V_gpio, 1);
		}
		else if (24 == volt) {
			gpio_direction_output (ntx_misc->cd_12V_gpio, 1);
			gpio_direction_output (ntx_misc->cd_N12V_EN_gpio, 0);
			gpio_direction_output (ntx_misc->cd_24V_gpio, 0);
			msleep (period);
			gpio_direction_output (ntx_misc->cd_24V_gpio, 1);
			gpio_direction_output (ntx_misc->cd_N12V_EN_gpio, 1);
		}
		else {
	    	printk ("Unknown voltage. Olny 12 & 24 voltage allowed.\n");
	    	return EINVAL;
		}
	}
    else {
    	printk ("Period not defined.\n Please enter \"Voltage period(ms)\".\n");
    	return EINVAL;
    }
	return count;
}
static DEVICE_ATTR(cash_drawer, 0666, cash_drawer_info, cash_drawer_ctl);

static ssize_t buzzer_info(struct device *dev, struct device_attribute *attr,
			char *buf)
{
    printk ("Setting buzzer frequency and period: freq period(ms)\n");   	
	return 0;
}

static ssize_t buzzer_ctl(struct device *dev, struct device_attribute *attr,
		       const char *buf, size_t count)
{
	int freq, period;
	char *pPeriod, *pFreq = buf;

    if (pPeriod = strchr (buf, ' ')) {
    	*pPeriod = '\0';
    	pPeriod++;
 		freq = simple_strtoul(pFreq, NULL, 10);
 		period = simple_strtoul(pPeriod, NULL, 10);
    	printk ("Setting buzzer %d, %dms\n",freq, period);   	
		if (!ntx_misc->buzzer_pwm) {
			ntx_misc->buzzer_pwm = pwm_request(1, "buzzer"); 	// pwm1 for buzzer
			if (IS_ERR(ntx_misc->buzzer_pwm)) {
				printk ("unable to request PWM for buzzer\n");
				return EINVAL;
			}
		}
		if (ntx_misc->buzzer_pwm) {
    		...
    		printk ("Setting buzzer %d\n", period_ns);   	
			pwm_config(ntx_misc->buzzer_pwm, period_ns>>1, period_ns);
			pwm_enable(ntx_misc->buzzer_pwm);
			msleep (period);
			pwm_config(ntx_misc->buzzer_pwm, 0, 366000);
			pwm_disable(ntx_misc->buzzer_pwm);
		}
    }
    else {
    	printk ("Period not defined.\n Please enter \"frequency period(ms)\".\n");
    	return EINVAL;
    }
	return count;
}
static DEVICE_ATTR(buzzer, 0666, buzzer_info, buzzer_ctl);

static ssize_t led_info(struct device *dev, struct device_attribute *attr,
			char *buf)
{
    printk ("led control. 0:off 1:on\n");   	
	return 0;
}

static ssize_t led_ctl(struct device *dev, struct device_attribute *attr,
		       const char *buf, size_t count)
{
    if (strchr (buf, '0')) {
 		up_write_reg (0x72, 0x0000);
    }
    else {
		up_write_reg (0x72, 0x0100);
    }
	return count;
}

static DEVICE_ATTR(led, 0666, led_info, led_ctl);

static ssize_t upg_key_info(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	*buf = gpio_get_value((32*2+26))?1:0;
    printk ("upgrade key value is %d\n",*buf);   	
	return 1;
}

static DEVICE_ATTR(upg_key, 0666, upg_key_info, NULL);
#endif//] CONFIG_MACH_MX6Q_NTX
...
static int pmic_battery_probe(struct platform_device *pdev)
{
	...
	// arm_pm_restart = pmic_restart;
	pm_power_off = msp430_power_off;
	arm_pm_restart = msp430_pm_restart;

	if (device_create_file(&(pdev->dev), &dev_attr_cash_drawer)) {
		pr_debug("Can't create cash drawer device file!\n");
		return -ENODEV;
	}
	
	if (device_create_file(&(pdev->dev), &dev_attr_buzzer)) {
		pr_debug("Can't create buzzer device file!\n");
		return -ENODEV;
	}

	if (device_create_file(&(pdev->dev), &dev_attr_led)) {
		pr_debug("Can't create led device file!\n");
		return -ENODEV;
	}
	
	if (device_create_file(&(pdev->dev), &dev_attr_upg_key)) {
		pr_debug("Can't create upggrade key file!\n");
		return -ENODEV;
	}
#endif //]CONFIG_MACH_MX6Q_NTX
	goto success;
...
#define	CASH_DRAWER_STATUS		0x700
#define	CASH_DRAWER_OPEN_12V  	0x701
#define	CASH_DRAWER_OPEN_24V  	0x702
#define	BUZZER_ON  		0x703
#define	BUZZER_OFF  	0x704
...
static int  ioctlDriver(struct file *filp, unsigned int command, unsigned long arg)
{
	...
	switch (command)
	{
	case CASH_DRAWER_STATUS:
		i = gpio_get_value (ntx_misc->cd_open_gpio);
		printk ("%s() : Cash drawer status -- %s\n",__func__,(i?"unlocked":"locked"));
		copy_to_user((void __user *)arg, &i, sizeof(unsigned long));
		break;
	case CASH_DRAWER_OPEN_12V:
		printk ("%s() : Open cash drawer 12V %d.\n",__func__,arg);
		gpio_direction_output (ntx_misc->cd_N12V_EN_gpio, 0);
		gpio_direction_output (ntx_misc->cd_24V_gpio, 1);
		gpio_direction_output (ntx_misc->cd_12V_gpio, 0);
		msleep (arg);
		gpio_direction_output (ntx_misc->cd_12V_gpio, 1);
		break;
	case CASH_DRAWER_OPEN_24V:
		printk ("%s() : Open cash drawer 24V %d.\n",__func__,arg);
		gpio_direction_output (ntx_misc->cd_12V_gpio, 1);
		gpio_direction_output (ntx_misc->cd_N12V_EN_gpio, 1);
		gpio_direction_output (ntx_misc->cd_24V_gpio, 0);
		msleep (arg);
	...
	case BUZZER_ON:
		...
		if (ntx_misc->buzzer_pwm) {
			pwm_config(ntx_misc->buzzer_pwm, 188000, 366000);
			pwm_enable(ntx_misc->buzzer_pwm);
			msleep (100);
			pwm_config(ntx_misc->buzzer_pwm, 0, 366000);
			pwm_disable(ntx_misc->buzzer_pwm);
		...
		break;
	default:
		printk ("%s() : Unsuppored command 0x%02X !\n",__func__,command);
		break;
	}
	return 0;
}
...
#define		DEVICE_NAME 		"ntx_io"
#define		DEVICE_MINOR		190

static struct miscdevice ntx_misc_Device = {
	.minor		= DEVICE_MINOR,
	.name		= DEVICE_NAME,
	.fops		= &driverFops,
};
...
#ifdef CONFIG_MACH_MX6Q_NTX//[
	int ret;
	ret = misc_register(&ntx_misc_Device);
	if (ret < 0) {
		printk("ntx_io: can't get major number\n");
		return ret;
	}
#endif //] CONFIG_MACH_MX6Q_NTX
...
#ifdef CONFIG_MACH_MX6Q_NTX//[
	misc_deregister(&ntx_misc_Device);
#endif //]CONFIG_MACH_MX6Q_NTX
}
