...
/**
 * halmac_mac_power_switch_8821c_sdio() - change mac power
 * @pHalmac_adapter
 * @halmac_power
...
#if 0
/**
 * halmac_tx_allowed_sdio_8821c() - check sdio tx reserved page
 * @pHalmac_adapter
 * @pHalmac_buf
 * @halmac_size
 * Author : Ivan Lin
...
)
{
	...
	PLATFORM_MSG_PRINT(pDriver_adapter, HALMAC_MSG_INIT, HALMAC_DBG_TRACE, "halmac_tx_allowed_sdio_8821c ==========>\n");
	...
	for (i = 0; i < tx_agg_num; i++) {
		...
		tx_required_page_num = (packet_size >> HALMAC_TX_PAGE_SIZE_2_POWER_8821C) + ((packet_size & (HALMAC_TX_PAGE_SIZE_8821C - 1)) ? 1 : 0);
...
/*oqt check flow need to be improvment, 20160316, Soar*/
/*
	counter = 10;
