...
#include "../../hal_halmac.h"	/* BIT_ACRC32_8821C, HALMAC_RX_DESC_SIZE_8821C and etc. */
...
static _pkt *prepare_recvframe_pkt(struct recv_buf *recvbuf, union recv_frame *recvframe)
{
	...
	if (rtl8821c_rx_fcs_appended(recvbuf->adapter) == _TRUE)
		skb_len -= IEEE80211_FCS_LEN;
	data = recvbuf->pdata + HALMAC_RX_DESC_SIZE_8821C + attrib->drvinfo_sz;
#if 0
...
static u8 recvbuf_handler(struct recv_buf *recvbuf)
{
	...
	while (ptr < recvbuf->ptail) {
		...
		rx_report_sz = HALMAC_RX_DESC_SIZE_8821C + attrib->drvinfo_sz;
		...
		if ((rtl8821c_rcr_check(p, BIT_ACRC32_8821C) == _FALSE)
		    && attrib->crc_err) {
		...
		if ((attrib->crc_err) || (attrib->icv_err)) {
			...
			ptr += HALMAC_RX_DESC_SIZE_8821C;
			if (rtl8821c_rx_ba_ssn_appended(p) == _TRUE)
...
s32 rtl8821cs_recv_hdl(_adapter *padapter)
{
	...
	do {
		...
		do {
			...
			/* fix Hardware RX data error, drop whole recv_buffer*/
			if ((!(pHalData->ReceiveConfig & BIT_ACRC32_8821C)) && pattrib->crc_err) {
			...
			/*if (rtl8821c_rx_ba_ssn_appended(p) == _TRUE)*/
			if (pHalData->ReceiveConfig & BIT_APP_BASSN_8821C)
				rx_report_sz = HALMAC_RX_DESC_SIZE_8821C + RTW_HALMAC_BA_SSN_RPT_SIZE + pattrib->drvinfo_sz;
			else
				rx_report_sz = HALMAC_RX_DESC_SIZE_8821C + pattrib->drvinfo_sz;
			...
			if ((pattrib->crc_err) || (pattrib->icv_err)) {
				...
				if (check_fwstate(&padapter->mlmepriv, WIFI_MONITOR_STATE) == _FALSE)
					if ((pattrib->pkt_rpt_type == NORMAL_RX) && (pHalData->ReceiveConfig & BIT_APP_FCS_8821C))
...
static void rtl8821c_recv_tasklet(void *priv)
{
	...
	do {
		...
		while (ptr < precvbuf->ptail) {
			...
			pkt_offset = HALMAC_RX_DESC_SIZE_8821C + pattrib->drvinfo_sz + pattrib->pkt_len;
			...
			if ((pattrib->crc_err) || (pattrib->icv_err)) {
				...
				recvframe_pull(precvframe, HALMAC_RX_DESC_SIZE_8821C + pattrib->drvinfo_sz);
				skb_pull(ppkt, HALMAC_RX_DESC_SIZE_8821C + pattrib->drvinfo_sz);
