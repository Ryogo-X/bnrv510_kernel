...
#include "../../hal_halmac.h"	/* rtw_halmac_sdio_tx_allowed(), HALMAC_TX_DESC_SIZE_8821C and etc. */
...
int rtl8821cs_get_tx_max_length(struct dvobj_priv *dvobj, u8 queue, u32 *size)
{
	...
	default:
		*size = (HALMAC_TX_FIFO_SIZE_8821C / 3);
		rtw_warn_on(1);
		break;
	}
#ifdef DBG_DUMP_RQPN
...
static s32 xmit_xmitframes(PADAPTER adapter, struct xmit_priv *pxmitpriv)
{
	...
	#endif
	u16 max_xmit_page;
	u8 txdesc_size = HALMAC_TX_DESC_SIZE_8821C;
	int inx[4];
	...
	for (idx = 0; idx < hwentry; idx++) {
		...
		if (0) {
			if (max_xmit_page != PageNum(max_xmit_len, HALMAC_TX_PAGE_SIZE_8821C)) {
				RTW_ERR("%s => max_xmit_page_1:%d != max_xmit_page_2:%d\n", __func__, max_xmit_page, PageNum(max_xmit_len, HALMAC_TX_PAGE_SIZE_8821C));
				rtw_warn_on(1);
			}
		}
		#endif
		...
		while (rtw_end_of_queue_search(sta_phead, sta_plist) == _FALSE) {
			...
			while (rtw_is_list_empty(frame_phead) == _FALSE) {
				...
				if ((NULL == pxmitbuf)				    
					#ifdef XMIT_BUF_SIZE
				    || ((pxmitbuf->pg_num + PageNum(txlen, HALMAC_TX_PAGE_SIZE_8821C)) > PageNum(max_xmit_len, HALMAC_TX_PAGE_SIZE_8821C))
					#else
				    || ((pxmitbuf->pg_num + PageNum(txlen, HALMAC_TX_PAGE_SIZE_8821C)) > max_xmit_page)
					#endif
				...
				if (ret == _FAIL) {
					...
					pxmitframe->pg_num = PageNum(txlen, HALMAC_TX_PAGE_SIZE_8821C);
					pxmitbuf->pg_num += PageNum(txlen, HALMAC_TX_PAGE_SIZE_8821C);
...
s32 rtl8821cs_mgnt_xmit(PADAPTER adapter, struct xmit_frame *pmgntframe)
{
	...
	txdesc_size = HALMAC_TX_DESC_SIZE_8821C;
	page_size = HALMAC_TX_PAGE_SIZE_8821C;

	rtl8821c_update_txdesc(pmgntframe, pmgntframe->buf_addr);

	pxmitbuf->len = txdesc_size + pattrib->last_txcmdsz;
	pxmitbuf->pg_num = PageNum(pxmitbuf->len, HALMAC_TX_PAGE_SIZE_8821C);
...
s32 rtl8821cs_init_xmit_priv(PADAPTER adapter)
{
	...
	_rtw_init_completion(&xmitpriv->sdio_xmit_thread_comp);
