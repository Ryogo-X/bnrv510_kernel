...
#include "rtl8821cs.h"		/* rtl8821cs_init_interrupt() */
#include "rtl8821cs.h"
...
static void rtl8821cs_init_xmit_info(_adapter *adapter)
{
	...
	RTW_INFO("%s => High Pages:%d, LOW Pages:%d, Normal Pages:%d, Extra Pages:%d, Pub Pages:%d, Total Pages:%d\n"
		 ...
		 , (HALMAC_TX_FIFO_SIZE_8821C / HALMAC_TX_PAGE_SIZE_8821C));
#endif

	hal_data->max_xmit_page = PageNum(MAX_XMITBUF_SZ, HALMAC_TX_PAGE_SIZE_8821C);
	...
	if (adapter->registrypriv.wifi_spec) {
		...
		#ifdef XMIT_BUF_SIZE
		hal_data->max_xmit_size_vovi = ((hal_data->tx_normal_page + hal_data->tx_pub_page) >> 1) * HALMAC_TX_PAGE_SIZE_8821C;
		hal_data->max_xmit_size_bebk = ((hal_data->tx_low_page + hal_data->tx_pub_page) >> 1) * HALMAC_TX_PAGE_SIZE_8821C;
		#endif
