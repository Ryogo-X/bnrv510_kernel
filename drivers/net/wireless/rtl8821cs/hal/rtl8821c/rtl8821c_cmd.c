...
#ifdef CONFIG_P2P
void rtl8821c_set_p2p_ps_offload_cmd(_adapter *adapter, u8 p2p_ps_state)
{
	...
	case P2P_PS_ENABLE:
		...
		if (pwdinfo->ctwindow > 0) {
			...
			rtw_write8(adapter, REG_CTWND_8821C, pwdinfo->ctwindow);
		}

		/* hw only support 2 set of NoA */
		for (i = 0; i < pwdinfo->noa_num; i++) {
			/* To control the register setting for which NOA */
			rtw_write8(adapter, REG_TXCMD_NOA_SEL_8821C, (i << 4));
			if (i == 0)
			...
			/* config NOA duration */
			rtw_write32(adapter, REG_NOA_PARAM_8821C, pwdinfo->noa_duration[i]);
			/* config NOA interval */
			rtw_write32(adapter, (REG_NOA_PARAM_8821C + 4), pwdinfo->noa_interval[i]);
			/* config NOA start time */
			rtw_write32(adapter, (REG_NOA_PARAM_8821C + 8), pwdinfo->noa_start_time[i]);
			/* config NOA count */
			rtw_write8(adapter, (REG_NOA_PARAM_8821C + 12), pwdinfo->noa_count[i]);
		}

		if ((pwdinfo->opp_ps == 1) || (pwdinfo->noa_num > 0)) {
			/* rst p2p circuit */
			rtw_write8(adapter, REG_P2P_RST_8821C, BIT(0));
...
static void c2h_ccx_rpt(PADAPTER adapter, u8 *pdata)
{
	...
	if (C2H_CCX_RPT_GET_RETRY_OVER(pdata) | C2H_CCX_RPT_GET_LIFE_TIME_OVER(pdata))
...
void c2h_handler_rtl8821c(PADAPTER adapter, u8 *pbuf, u16 length)
{
	...
	pc2h_data = pbuf + HALMAC_RX_DESC_SIZE_8821C + 2;
