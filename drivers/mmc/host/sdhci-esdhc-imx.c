...
static u32 esdhc_readl_le(struct sdhci_host *host, int reg)
{
	...
	if (unlikely(reg == SDHCI_PRESENT_STATE)) {
		...
		else if	(imx_data->flags & ESDHC_FLAG_GPIO_FOR_CD_WP|ESDHC_CD_WIFI_PWR) {
...
#if defined(CONFIG_MACH_MX6SL_NTX) || defined(CONFIG_MACH_MX6Q_NTX) //[
...
#endif //] defined(CONFIG_MACH_MX6SL_NTX) || defined(CONFIG_MACH_MX6Q_NTX)
		{
