...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	if(0==gptHWCFG->m_val.bUIStyle) {
		/* single touch event for ebrmain*/
		input_set_abs_params(md->input, ABS_X, 0, max_x, 0, 0);
		input_set_abs_params(md->input, ABS_Y, 0, max_y, 0, 0);
		input_set_abs_params(md->input, ABS_PRESSURE, 0, max_p, 0, 0);
	}
...
int cyttsp5_mt_probe(struct device *dev)
{
	...
	/* Flip the Y axis in Android platfrom */
	if(2==gptHWCFG->m_val.bUIStyle) {
		printk("%s: Reverse mt platform flag INV_Y in Android platform\n",__func__);
		mt_pdata->flags ^= CY_MT_FLAG_INV_Y;
	}
